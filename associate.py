...
This script is adapted from the TUM RGB-D dataset tools.
"""
...
def read_file_list(filename):
    ...
    File format:
    The file format is "timestamp filename"
    """
    file = open(filename)
    data = file.read()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "first_file", help="first text file (format: timestamp filename)"
    )
    parser.add_argument(
        "second_file", help="second text file (format: timestamp filename)"
    )
    parser.add_argument(
        "--first_only",
    ...
    if args.first_only:
        for a, b in matches:
            print("%f %s" % (a, first_list[a]))
    else:
        for a, b in matches:
            print("%f %s %f %s" % (a, first_list[a], b, second_list[b]))
